usuario = input ("Boa tarde (づ￣ 3￣)づ. Qual seu nome? ")

print (f"Olá {usuario} o(*^▽^*)┛!\nO Paulo me programou para demonstrar todos os conhecimentos que ele está adiquirindo no curso de Engenharia de Computação!")

print("Primeiramente vou fazer uma brincadeira com vc.\nDigite algumas informações necessárias para que eu descubra sua idade (⌐■_■) ")

dia = input ("Q dia você nasceu?  ")
dia = int (dia)

mes = input ("Agora me conte o mês: ")
mes = int (mes)

ano = input ("E o ano: ")
ano = int (ano)

print (f"Certo! então você nasceu em {dia}/{mes}/{ano} não é mesmo? (ToT)/~~~ ")
input ()
print (f"Já sei sua idade {usuario}! Quer que eu diga ou não? (o゜▽゜)o☆ Hehe ")
input ()
print (f"Vou te dizer. Mas você vai ficar me devendo um Pix, pois meu desenvolvedor quer sair com as gatinhas! Logo ele precisa de din din (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧ Ok?")
input ()
print (f"{usuario} tem {2021 - ano} anos. Um jovem senhor kkk. É isso {usuario}, agora vou me retirar pois estou cansado e preciso recarregar as energias. Até uma próxima execução (〃￣︶￣)人(￣︶￣〃).") 

